...
function createTransformRegExp(
  ...
  return new RegExp(
    Object.keys(rules)
      .map((rule) => escapeRegExp(`${needEscape ? escapePrefix : ''}${rule}`))
      .join('|'),
    'g'
  );
}
...
export const cacheTransformSelector = (function () {
  ...
  return (selector = '', rules = defaultRules) => {
    if (!transformRegExp) transformRegExp = createTransformRegExp(rules);
 ...
 * @example bg-\[\#452233\]\:50 => bg--fl--w-452233-fr--c-40-c-50
 */
export function transformEscapESelector(
  selector = '',
  rules = defaultRules
): string {
...
export const cacheTransformEscapESelector = (function () {
  ...
  return (selector = '', rules = defaultRules) => {
    if (!transformRegExp) transformRegExp = createTransformRegExp(rules, true);
...
export const cacheRestoreSelector = (function () {
  ...
  return (selector = '', rules = defaultRules) => {
    if (!transformRegExp) {
      reverseRules = Object.fromEntries(
        Object.entries(rules).map(([key, value]) => [value, key])
      );
